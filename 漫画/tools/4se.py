...
def getchapterlist(page):
    try:
        url = 'https://www.jao4.com/tupian/list-%E5%8D%A1%E9%80%9A%E5%8A%A8%E6%BC%AB-{}.html'.format(page)
        #print("=================开始下载第{}页=================".format(page))
        html = http(url)
        if html == "":
            raise TypeError
        div = bs(html,'html.parser').find_all('div',class_="box list channel max-border list-text-my")
        li = bs(str(div[0]),'html.parser').find_all('ul')
        a = bs(str(li),'html.parser').find_all('a')
        with ThreadPoolExecutor(thread_num) as t:
            for each in a:
                title = each.get('title')
                chapterurl = root + each.get('href')
...
def getpiclist(url,title,mhid):
    ...
    try:
        ...
        if html == "":
            raise TypeError
        img = bs(html,'html.parser').find_all('img',class_="videopic lazy")
        with ThreadPoolExecutor(thread_num) as t:
            for i in range(len(img)):
                imgurl = img[i].get('data-original')
                if local:
                    #本地保存
